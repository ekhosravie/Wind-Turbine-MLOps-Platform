"""
Generate synthetic data. Verbatim from original cells 6+8+9 (function-based, already reasonably clean). Depends on config (cell 3) and spark session being in scope when run as a Databricks notebook.
"""

# ======================================================================================
# COMPREHENSIVE WIND TURBINE SYNTHETIC DATA GENERATOR
# Implements the full 40-point specification with 6 related datasets
# ======================================================================================
...
print("="*80)
print("WIND TURBINE DATA GENERATION - COMPREHENSIVE MULTI-DATASET APPROACH")
print("="*80)
print(f"Configuration:")
print(f"  Farms: {config.num_farms}")
print(f"  Turbines: {config.num_turbines} ({config.turbines_per_farm} per farm)")
print(f"  Date Range: {config.start_date} to {config.end_date}")
print(f"  Frequency: {config.sensor_interval_minutes} minutes")
print(f"  Random Seed: {config.random_seed} (deterministic)")
print("="*80)

# NOTE: This implementation demonstrates the architecture and key concepts.
# For the FULL 3.15M record dataset, adjust config.data_months and run in production.
# Current configuration generates a functional demonstration dataset.

print("\n[Module 1/6] Generating Turbine Master Data...")
...
# Generate the data
print("\n[Module 1/6] Generating Synthetic Data...\n")
synthetic_data = generate_wind_turbine_data(config)

print("\n[Preview] Sample SCADA Records:")
display(synthetic_data.head(10))

print("\n[Preview] Failure Events:")
if len(failure_events_data) > 0:
    display(failure_events_data.head(10))
else:
    print("No failure events generated (increase data_months or failure_rate)")
# Save generated datasets to temporary source tables for Bronze ingestion
from pyspark.sql.types import *
from pyspark.sql import functions as F

print("[Module 6/6] Saving datasets to source tables...\n")

# === SCADA Sensor Data ===
# Convert pandas to Spark DataFrame
scada_spark_df = spark.createDataFrame(synthetic_data)

# Save to temp table (simulates source system)
source_scada_table = f"{config.catalog}.bronze.source_turbine_sensor_scada"
scada_spark_df.write.format("delta").mode("overwrite").saveAsTable(source_scada_table)
print(f"✓ Saved {scada_spark_df.count():,} SCADA records to {source_scada_table}")

# === Failure Events ===
if len(failure_events_data) > 0:
    failure_spark_df = spark.createDataFrame(failure_events_data)
    source_failure_table = f"{config.catalog}.bronze.source_failure_events"
    failure_spark_df.write.format("delta").mode("overwrite").saveAsTable(source_failure_table)
    print(f"✓ Saved {failure_spark_df.count():,} failure events to {source_failure_table}")
else:
    print("⚠ No failure events to save")

# === Maintenance Events ===
if len(maintenance_events_data) > 0:
    maintenance_spark_df = spark.createDataFrame(maintenance_events_data)
    source_maintenance_table = f"{config.catalog}.bronze.source_maintenance_events"
    maintenance_spark_df.write.format("delta").mode("overwrite").saveAsTable(source_maintenance_table)
    print(f"✓ Saved {maintenance_spark_df.count():,} maintenance events to {source_maintenance_table}")
else:
    print("⚠ No maintenance events to save")

print("\n" + "="*80)
print("SYNTHETIC DATA GENERATION COMPLETE")
print("="*80)
print("\nNext Steps:")
print("  1. Bronze Layer: Ingest from source tables with audit trail")
print("  2. Silver Layer: Clean, validate, and score data quality")
print("  3. Gold Layer: Create business KPIs and ML-ready datasets")
print("  4. Feature Engineering: Generate predictive features")
print("  5. Model Training: Train and select Champion model")
print("="*80)

# Quick validation
print("\n[Validation] Dataset Statistics:")
scada_spark_df.select(
    F.count('*').alias('total_records'),
    F.countDistinct('turbine_id').alias('unique_turbines'),
    F.countDistinct('farm_id').alias('unique_farms'),
    F.min('timestamp').alias('start_date'),
    F.max('timestamp').alias('end_date'),
    F.sum(F.col('failure_flag')).alias('failure_records'),
    F.round(F.avg('active_power_kw'), 2).alias('avg_power_kw')
).show(truncate=False)